    ...
    for f_name in F_NAME:
        ...
        with open(f_name) as inf:
            for line in inf:
                ...
                if (g_info['fname'] not in name_dict):  #判断函数名是否存在于名字字典里
                    name_dict[g_info['fname']] = name_num
                    name_num += 1
        #if i == 1:
        #    break
        #i += 1
    return name_dict  #为函数名创建一个字典索引
    ...
    for f_name in F_NAME:
        with open(f_name) as inf:
            for line in inf:
                ...
                classes[label].append(len(graphs))  #给不同编译环境下相同的函数分配一个id
...
def get_pair(Gs, classes, M, st = -1, output_id = False, load_id = None):
    ...
    if load_id is None:
        ...
        if (st + M > len(Gs)):  #处理数据不够一个完整的epoch情况
            M = len(Gs) - st
        ed = st + M
        ...
        for g_id in range(st, ed):
            ...
            if (len(classes[cls]) >= 2):    #当超过两个（包括）的时候，随机选择一个图作为positive
                g1_id = classes[cls][np.random.randint(tot_g)]
                while g_id == g1_id:        #当选取的两个相同的时候继续选取。
                    g1_id = classes[cls][np.random.randint(tot_g)]
                pos_ids.append( (g_id, g1_id) )

            cls2 = np.random.randint(C)  #从其他选择来作为负样本
    ...
    maxN1 = 0
    maxN2 = 0
    for pair in pos_ids:    #找出包含最多的节点数量
    ...
    X1_input = np.zeros((M, maxN1, feature_dim), dtype='float16') #生成M个maxN1 * feature_dim的数组
    X2_input = np.zeros((M, maxN2, feature_dim), dtype='float16')
    node1_mask = np.zeros((M, maxN1, maxN1), dtype='float16')
    node2_mask = np.zeros((M, maxN2, maxN2), dtype='float16')
...
def train_epoch(model, graphs, classes, batch_size, load_data=None):
    ...
    perm = np.random.permutation(len(epoch_data))   #Random shuffle #随机排列序列。
