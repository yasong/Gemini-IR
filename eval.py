...
if __name__ == '__main__':
    ...
    if LIB == 'SSL':
        ...
        npy_name = 'data/acfgSSL_{}_'.format(NODE_FEATURE_DIM)
        res_name = 'res/acfgSSL_{}_'.format(NODE_FEATURE_DIM)
        LOAD_PATH = './saved_model/acfgSSL_{}/graphnn-model_best'.format(NODE_FEATURE_DIM)
        ...
        LOAD_PATH = './saved_model/acfgSSL_{}_vex/graphnn-model_best'.format(NODE_FEATURE_DIM)
        SOFTWARE=('openssl-1.0.1f-', 'openssl-1.0.1u-')
        COMPILER=('armeb-linux', 'i586-linux', 'mips-linux')
        ...
        LOAD_PATH = './saved_model/acfgBin_{}/graphnn-model_best'.format(NODE_FEATURE_DIM)
        ...
        LOAD_PATH = './saved_model/acfgBin_{}_vex/graphnn-model_best'.format(NODE_FEATURE_DIM)
        SOFTWARE=('binutils-2.26-', 'binutils-2.28-')
        COMPILER=('armeb-linux', 'i586-linux', 'mips-linux', 'powerpc-linux')
        ...
        LOAD_PATH = './saved_model/acfgCore_{}/graphnn-model_best'.format(NODE_FEATURE_DIM)
    ...
    if os.path.isfile(npy_name + 'test.json'):
        with open(npy_name + 'test.json') as inf:
        ...
        with open(npy_name + 'test.json', 'w') as outf:
    ...
    gnn.say( "fpr on validation set: {}".format(fpr) )
    gnn.say( "tpr on validation set: {}".format(tpr) )

    #np.save('res/val_fpr.npy', fpr)
    #np.save('res/val_tpr.npy', tpr)
    ...
    np.save(res_name + 'test_fpr.npy', fpr)
    np.save(res_name + 'test_tpr.npy', tpr)

    #np.save('res/vex_test_fpr.npy', fpr)
    #np.save('res/vex_test_tpr.npy', tpr)
    #plot_roc(fpr, tpr)
